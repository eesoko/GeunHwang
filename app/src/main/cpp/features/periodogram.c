                 ...
                 emxArray_real_T *Px, emxArray_real_T *w)
{
  ...
  if (xw->size[0] > obj_next_next_value) {
    ...
    for (k = 0; k < nFullPasses; k++) {
      ...
      n = ((int)obj_next_next_value / 2) << 1;
      n2 = n - 2;
      for (b_k = 0; b_k <= n2; b_k += 2) {
        __m128d r1;
        r = _mm_loadu_pd(&wrappedData_data[b_k]);
        r1 = _mm_loadu_pd(&xw_data[acoef + b_k]);
        _mm_storeu_pd(&wrappedData_data[b_k], _mm_add_pd(r, r1));
      }
      for (b_k = n; b_k < loop_ub; b_k++) {
  ...
  if (b_xw->size[0] == 0) {
    ...
    costab1q_data[0] = 1.0;
    acoef = (int)((unsigned int)n >> 1) - 1;
    for (k = 0; k <= acoef; k++) {
      costab1q_data[k + 1] = cos(f * ((double)k + 1.0));
    }
    acoef += 2;
    for (k = acoef; k < n; k++) {
      costab1q_data[k] = sin(f * (double)(n - k));
    }
    costab1q_data[n] = 0.0;
    ...
    if (!useRadix2) {
      ...
      c_FFTImplementationCallback_dob(b_xw, nFullPasses,
      ...
      for (k = 0; k < n; k++) {
        select_data[k + 1] = costab1q_data[k + 1];
        wrappedData_data[k + 1] = -costab1q_data[(n - k) - 1];
      }
      acoef = costab1q->size[1];
      for (k = acoef; k <= nFullPasses; k++) {
        select_data[k] = -costab1q_data[nFullPasses - k];
        wrappedData_data[k] = -costab1q_data[k - n];
      }
      csz_idx_0 = (int)obj_next_next_value;
      acoef = Xx->size[0];
      Xx->size[0] = (int)obj_next_next_value;
